...
//
// The HKDF context labels used in master-key derivation
// (`mosey-cert-master`) and cert signing (`mosey-cert-v1`) are a
// commitment, not an implementation detail: once a workspace master
// has been minted they are baked into every cert ever derived from
// it. Changing them silently breaks every existing workspace; only
// touch them with a deliberate version bump and a migration story.
package cert
