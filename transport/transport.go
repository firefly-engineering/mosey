// Package transport is the wire-backend abstraction every mosey
// upper layer dials through. A [Transport] gives out [Stream]s
// (bidi byte channels); [Multi] aggregates several backends behind
// one interface so a single attach or vterm can speak libp2p,
// HTTP/2, WebSocket and unix-domain sockets at the same time. The
// auth wrap, vterm session, and attach client all program against
// these types — never against a specific backend.
package transport
